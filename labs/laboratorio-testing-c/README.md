...
> R: Inicializa el carrito para poder usarlo de forma segura
...
> R: Deberia multiplicar por la cantidad.    total += c->items[i].precio * c->items[i].cantidad;
...
> R: if (c->cantidad <= MAX_ITEMS) es el incorrecto. Deberia ser: (c->cantidad < MAX_ITEMS)
